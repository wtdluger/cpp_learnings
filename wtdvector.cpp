...
int main()
{
    std::vector<double> t = {9.3, 9.1, 9.87};
    std::vector<double> u = {8.7, 5.2, 8.99};
    // std::vector<int> v;

    std::vector<double> a = {0.1, 0.2, 0.3};
    std::vector<std::string> b = {"The", "Fox", "Moo"};

    // std::vector<double> cp = crossProduct(t,u);
    // double dp = dotProduct(t,u);
    ...
    printVector(a);
    printVector(b);
    cout << "Vector sizes: " << t.size() << ", " << u.size() << endl;
    printVector(cp);
    cout << dp << endl;
